...
salvar_figura <- function(nome_arquivo, plot = last_plot(), 
  ...
  ggsave(
    filename = paste0("figuras/acordabrasil/", nome_arquivo, ".png"),
    plot     = plot,
    width    = largura,
    height   = altura,
    units    = unidade,
...
acordabrasil_setembro <- fromJSON("Dados/Telegram/Grupo - Acorda Brasil/Setembro 2025/result.json") %>% 
  .[["messages"]] %>%
  select(any_of(c("id", "date", "text", "forwarded_from")))
acordabrasil_outubro <- fromJSON("Dados/Telegram/Grupo - Acorda Brasil/Outubro 2025/result.json") %>% 
  .[["messages"]]%>%
  select(any_of(c("id", "date", "text", "forwarded_from")))
acordabrasil_novembro <- fromJSON("Dados/Telegram/Grupo - Acorda Brasil/Novembro 2025/result.json") %>% 
  .[["messages"]]%>%
  select(any_of(c("id", "date", "text", "forwarded_from")))
...
acordabrasil <- bind_rows(acordabrasil_setembro, acordabrasil_outubro, acordabrasil_novembro)

rm(acordabrasil_setembro, acordabrasil_outubro, acordabrasil_novembro)
...
mensagens_encaminhadas <- acordabrasil %>%
...
write_xlsx(contagem_paises, "paises_mais_citados_grupoacordabrasil.xlsx")
...
salvar_figura("acordabrasil_top10_paises")
...
write_xlsx(contexto_paises_por_canal, "paises_contexto_por_canal_grupoacordabrasil.xlsx")
...
salvar_figura("acordabrasil_mapa")
...
write_xlsx(mensagens_multi_paises, "mensagens_multi_paises_grupoacordabrasil.xlsx")
...
salvar_figura("acordabrasil_coocorrencia")

write_xlsx(pares_paises, "coocorrencia_paises_grupoacordabrasil.xlsx")
...
ggraph(dados_grafo, layout = "fr") +
  ...
  labs(title = paste0("Teia Discursiva: ",palavra_titulo),
       subtitle = "Termos mais correlacionados no grupo Acorda Brazil")

salvar_figura(paste0("acordabrasil2_teia_discursiva_",palavra))
...
wordcloud_pais("china", top_n = 100)
