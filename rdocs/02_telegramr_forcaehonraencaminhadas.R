...
salvar_figura <- function(nome_arquivo, plot = last_plot(), 
  ...
  ggsave(
    filename = paste0("figuras/forcaehonra/", nome_arquivo, ".png"),
    plot     = plot,
    width    = largura,
    height   = altura,
    units    = unidade,
...
forcaehonra_setembro <- fromJSON("Dados/Telegram/Grupo - FORÇA & HONRA chat/Setembro 2025/result.json") %>% 
  .[["messages"]] %>%
  select(any_of(c("id", "date", "text", "forwarded_from")))
forcaehonra_outubro <- fromJSON("Dados/Telegram/Grupo - FORÇA & HONRA chat/Outubro 2025/result.json") %>% 
  .[["messages"]]%>%
  select(any_of(c("id", "date", "text", "forwarded_from")))
forcaehonra_novembro <- fromJSON("Dados/Telegram/Grupo - FORÇA & HONRA chat/Novembro 2025/result.json") %>% 
  .[["messages"]]%>%
  select(any_of(c("id", "date", "text", "forwarded_from")))
...
forcaehonra <- bind_rows(forcaehonra_setembro, forcaehonra_outubro, forcaehonra_novembro)

rm(forcaehonra_setembro, forcaehonra_outubro, forcaehonra_novembro)
...
mensagens_encaminhadas <- forcaehonra %>%
...
write_xlsx(contagem_paises, "paises_mais_citados_grupoforcaehonra.xlsx")
...
salvar_figura("forcaehonra_top10_paises")
...
write_xlsx(contexto_paises_por_canal, "paises_contexto_por_canal_grupoforcaehonra.xlsx")
...
salvar_figura("forcaehonra_mapa")
...
write_xlsx(mensagens_multi_paises, "mensagens_multi_paises_grupoforcaehonra.xlsx")
...
salvar_figura("forcaehonra_coocorrencia")

write_xlsx(pares_paises, "coocorrencia_paises_grupoforcaehonra.xlsx")
...
palavras_limpas <- mensagens_encaminhadas_paises %>%  #troquei de mensagens_encaminhadas p/ paises
  mutate(
...
ggraph(dados_grafo, layout = "fr") +
  ...
  labs(title = paste0("Teia Discursiva: ",palavra_titulo),
       subtitle = "Termos mais correlacionados no grupo Força e Honra")

salvar_figura(paste0("forcaehonra2_teia_discursiva_",palavra))
...
#países de palavras compostas, aqui!!, separar com espaço
wordcloud_pais("russia", top_n = 100)
